...
void cpc_svn_export(vector<int>& svn_nums,string submit_path,vector<string>& exportfilelist){
    ...
    cout<<submit_path<<endl;

    for (int svn_num: svn_nums) {
        getCPCSvnInfoAndDownloadFilesFromCmd(svn_num,submit_path);
    }

    vector<string> filelist;

    getAllFiles(submit_path+"\\trunk", filelist);

    int subcount=submit_path.size();

    ofstream outfile(submit_path+"\\cpc_export_file_list.txt", ios::out);
    for (int svn_num: svn_nums) {
        outfile << svn_num << " ";
    }
    outfile << endl;
    ...
    cout << "svn导出文件列表位置："+submit_path+"\\cpc_export_file_list.txt" << endl;
    cout<<"svn文件导出完成...."<<endl;
}
...
void getCPCSvnInfoAndDownloadFilesFromCmd(int version,string submit_path) {
    ...
    string showdiffcmd = "svn diff -r " + to_string(version - 1) + ":" + to_string(version) +
                         " --summarize http://vcs.comac.intra/svn/cpc";
    ...
    for (int i = 0; i < changeList.size(); ++i) {
        ...
        string path = changeInfoList.at(1).substr(30);
        string svnExportPath = "http://vcs.comac.intra/svn/cpc" + path;
